...
export function loadRodizioResumo() {
  ...
  `);
  const ped = sumOr0(`
    SELECT IFNULL(SUM(COALESCE(valor_entregue_cents, valor_cents, 0)),0) AS s
    FROM peditorios
  `);
  const pat = sumOr0(`SELECT IFNULL(SUM(valor_entregue_cents),0) AS s FROM patrocinadores`);
  const leiloes = sumOr0(`
    SELECT IFNULL(SUM(valor_recebido_cents),0) AS s
    FROM leiloes
    WHERE numero BETWEEN 1 AND 4
  `);
  const lugares = sumOr0(`
    SELECT IFNULL(SUM(valor_pago_cents),0) AS s
    FROM vendas_lugares
  `);
  ...
  const saldoMovimentos = recMov - despMov + ped + pat + leiloes + lugares;
